...
zip_files = glob(os.path.join(outDIR, '*.zip'))
for zip_file in zip_files:
    #print(zip_file)
    fileDate3 = datetime.now().strftime("_%Y%m%d%H%M%S%f")
    newname = zip_file + fileDate3
    os.rename(os.path.join(outDIR, zip_file), os.path.join(outDIR,newname))
