...
outDIR = (r'\\atl-va-fs06\data\Vault\2021')
filename = os.path.join(outDIR, 'Vault_TAT_' + fileDate)
...
#and RMN like '101%')
#and d.ImageDateTime > '2021-04-01'
...
finale = finale.loc[finale['Status'] == 'Partial']
...
writer = pd.ExcelWriter(r'\\atl-va-fs06\data\Vault\2021\Vault_TAT_' + fileDate + '.xlsx', engine='xlsxwriter')


# In[18]:


finale.to_excel(writer, sheet_name='Outstanding RMNs', index=False)
...
worksheet = writer.sheets['Outstanding RMNs']
...
options = {'style': 'Table Style Medium 1',
          'columns': [{'header': 'RMN'},
                     {'header': 'TrackingNo'},
                     {'header': 'ClaimType'},
                     {'header': 'TotalBatches'},
...
body = 'Please find the latest Vault TAT summary within attached spreadsheet.'
